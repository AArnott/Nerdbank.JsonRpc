...
public partial class JsonRpcServerTests : TestBase
{
	...
	public async Task SimpleRequestResponse()
	{
		...
		int resultValue = ((MessagePackSerializerPlugin)((JsonRpcClient)this.jsonRpc).Serializer).Serializer.Deserialize(result.Result.AsMessagePack(), PolyType.SourceGenerator.TypeShapeProvider_Nerdbank_JsonRpc_Tests.Default.Int32, this.TimeoutToken);
	...
	private JsonRpcRequest CreateCancellationRequest(RequestId id)
	{
		...
		MessagePackWriter msgpackWriter = new(seq);
		msgpackWriter.WriteArrayHeader(1);
		((MessagePackSerializerPlugin)((JsonRpcClient)this.jsonRpc).Serializer).Serializer.Serialize(ref msgpackWriter, id, PolyType.SourceGenerator.TypeShapeProvider_Nerdbank_JsonRpc_Tests.Default.RequestId, this.TimeoutToken);
		msgpackWriter.Flush();
